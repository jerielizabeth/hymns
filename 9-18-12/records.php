<?php
        ...
        */
        ...
        function renderForm($personName = '', $gender ='', $birth = '', $death = '', $id = '')
        { ?>
                        ...
                        <body>
                                ...
                                <div>
                                        ...
                                        <strong>Author Name: *</strong> <input type="text" name="personName"
                                                value="<?php echo $personName; ?>"/><br/>
                                        <strong>Gender: *</strong> <input type="text" name="gender"
                                                value="<?php echo $gender; ?>"/>
                                        <strong>Year of Birth: *</strong> <input type="text" name="birthYear"
                                                value="<?php echo $birth; ?>"/>
                                        <strong>Year of Death: *</strong> <input type="text" name="deathYear"
        ...
        if (isset($_GET['id']))
        {
                ...
                if (isset($_POST['submit']))
                {
                        ...
                        if (is_numeric($_POST['id']))
                        {
                                ...
                                $id = $_POST['id'];
                                $personName = htmlentities($_POST['personName'], ENT_QUOTES);
                                ...
                                if ($personName == ''|| $gender == '')
                                {
                                        // if they are empty, show an error message and display the form
                                        $error = 'ERROR: Please fill in all required fields!';
                                        renderForm($personName, $gender, $birth, $death, $error, $id);
                                }
                                else
                                {
                                        ...
                                        if ($stmt = $mysqli->prepare("UPDATE person SET personName = ?, gender = ?, birthYear = ?, deathYear = ?
                                                WHERE id=?"))
                                        {
                                                $stmt->bind_param("ssi", $personName, $gender, $birth, $death $id);
                                                $stmt->execute();
                                                $stmt->close();
                                        }
                ...
                else
                {
                        ...
                        if (is_numeric($_GET['id']) && $_GET['id'] > 0)
                        {
                                ...
                                if($stmt = $mysqli->prepare("SELECT * FROM person WHERE pk_person=?"))
                                {
                                        ...
                                        $stmt->bind_result($id, $personName, $gender, $birth, $death);
                                        $stmt->fetch();
                                        
                                        // show the form
                                        renderForm($personName, $gender, $birth, $death, NULL, $id);
        ...
        else
        {
                ...
                if (isset($_POST['submit']))
                {
                        ...
                        if ($personName == '' || $gender == '')
                        {
                                // if they are empty, show an error message and display the form
                                $error = 'ERROR: Please fill in all required fields!';
                                renderForm($personName, $gender, $birth, $death, $error);
                        }
                        else
                        {
